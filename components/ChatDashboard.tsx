...
import { Search, Send, Image as ImageIcon, MoreVertical, Phone, Video, Loader2, Globe, Users, MessageCircle, User } from 'lucide-react'
...
// --- MAIN COMPONENT ---
export default function ChatDashboard({ session, onCall }: { session: any, onCall: (target: any, isVideo: boolean) => void }) {
    ...
    useEffect(() => {
        const loadData = async () => {
            ...
            // --- A. FETCH FRIENDS (To show even if no chat exists) ---
            ...
            // --- B. FETCH ACTIVE CHATS (Sellers/Others + History) ---
            ...
            const chatHistory = new Map() // Map<PartnerID, {lastMsg, time}>
            ...
            // --- C. MERGE LISTS (Friends + Active Chats) ---
            // 1. Get all unique User IDs (Friends + Chat Partners)
            ...
            if (allUserIds.size > 0) {
                ...
                if (profiles) {
                    ...
                    // Sort: Active chats first, then alphabetical friends
                    threads.sort((a, b) => {
            ...
            if (groups) {
                setGroupThreads(groups.map(g => ({
                    id: g.id,
                    name: g.name,
                    image: g.image_url,
                    type: 'group',
                    lastMsg: 'Community Chat'
                })))
            }
    ...
    useEffect(() => {
        ...
        const fetchMessages = async () => {
            ...
            if (activeThread.type === 'direct') {
                query = query.or(`and(sender_id.eq.${session.user.id},receiver_id.eq.${activeThread.id}),and(sender_id.eq.${activeThread.id},receiver_id.eq.${session.user.id})`)
            } else if (activeThread.type === 'group') {
                query = query.eq('group_id', activeThread.id)
            } else if (activeThread.type === 'global') {
            ...
            <div className="w-80 border-r border-zinc-100 flex flex-col bg-zinc-50/50">
                <div className="p-4 space-y-4">
                    ...
                    <div className="relative">
                        ...
                        <Input 
                            placeholder="Search..." 
                            className="pl-9 bg-white border-none shadow-sm rounded-xl" 
                            value={search}
                ...
                <div className="flex-1 overflow-y-auto custom-scrollbar pt-2 px-2">
                    ...
                    {activeTab === 'direct' && (
                        filteredDirect.length > 0 ? filteredDirect.map(t => (
                            <ThreadItem key={t.id} item={t} active={activeThread?.id === t.id} onClick={() => setActiveThread(t)} icon={MessageCircle} />
                        )) : !loading && <div className="text-center text-xs text-zinc-400 mt-10">No friends or chats found.</div>
                    )}

                    {activeTab === 'groups' && (
                        filteredGroups.length > 0 ? filteredGroups.map(t => (
                            <ThreadItem key={t.id} item={t} active={activeThread?.id === t.id} onClick={() => setActiveThread(t)} icon={Users} />
                        )) : !loading && <div className="text-center text-xs text-zinc-400 mt-10">No groups joined.</div>
                    )}

                    {activeTab === 'global' && (
                        <ThreadItem 
                            item={{ name: 'Global Public Chat', type: 'global', lastMsg: 'Chat with everyone!' }} 
                            active={activeThread?.type === 'global'} 
                            onClick={() => setActiveThread({ id: 'global', name: 'Global Chat', type: 'global', image: null })}
                            icon={Globe}
                        />
                    )}
                </div>
            ...
            <div className="flex-1 flex flex-col bg-white">
                {activeThread ? (
                    <>
                        <div className="h-16 border-b border-zinc-50 flex items-center justify-between px-6 bg-white/80 backdrop-blur-md sticky top-0 z-10">
                            <div className="flex items-center gap-3">
                                <Avatar className="h-10 w-10 border border-zinc-100">
                                    <AvatarImage src={activeThread.image} />
                                    <AvatarFallback className="bg-yellow-100 text-yellow-700 font-bold">{activeThread.name[0]}</AvatarFallback>
                                </Avatar>
                                <div>
                                    <h3 className="font-bold text-sm text-zinc-900 flex items-center gap-2">
                                        {activeThread.name}
                                        {activeThread.type === 'group' && <span className="bg-zinc-100 text-zinc-500 text-[9px] px-1.5 py-0.5 rounded-md">GROUP</span>}
                                        {activeThread.isFriend && <span className="bg-green-100 text-green-600 text-[9px] px-1.5 py-0.5 rounded-md">FRIEND</span>}
                                    </h3>
                                    {activeThread.type === 'direct' && <span className="text-[10px] text-zinc-400 flex items-center gap-1">{activeThread.isFriend ? 'Connected' : 'Active Chat'}</span>}
                                </div>
                            </div>
                            {activeThread.type === 'direct' && (
                                <div className="flex items-center gap-1">
                                    <Button variant="ghost" size="icon" onClick={() => onCall(activeThread, false)} className="text-zinc-400 hover:text-zinc-900"><Phone className="w-5 h-5"/></Button>
                                    <Button variant="ghost" size="icon" onClick={() => onCall(activeThread, true)} className="text-zinc-400 hover:text-zinc-900"><Video className="w-5 h-5"/></Button>
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-4 bg-zinc-50/30" ref={scrollRef}>
                            {messages.length === 0 && (
                                <div className="h-full flex flex-col items-center justify-center text-zinc-300">
                                    ...
                                    <p className="text-sm">Start chatting with {activeThread.name}</p>
                                </div>
                            )}
                                    ...
                                    <div key={msg.id} className={`flex gap-3 animate-in slide-in-from-bottom-2 ${isMe ? 'flex-row-reverse' : ''}`}>
                                        {!isMe && (
                                            <Avatar className="h-8 w-8 mt-1 border border-white shadow-sm">
                                                ...
                                                <AvatarFallback className="text-[10px] bg-zinc-200">{msg.profiles?.username?.[0]}</AvatarFallback>
                                            </Avatar>
                                        )}
                                        <div className={`max-w-[70%] rounded-2xl px-4 py-2.5 text-sm shadow-sm ${isMe ? 'bg-zinc-900 text-white rounded-tr-none' : 'bg-white border border-zinc-100 text-zinc-800 rounded-tl-none'}`}>
                                            {!isMe && activeThread.type !== 'direct' && <p className="text-[10px] font-bold opacity-50 mb-1">{msg.profiles?.username}</p>}
                                            <p className="leading-relaxed">{msg.content}</p>
                                            {msg.media_url && <img src={msg.media_url} className="mt-2 rounded-lg max-h-48 object-cover" />}
                                        </div>
                                    </div>
                                )
                            })}
                        ...
                        <div className="p-4 border-t border-zinc-50 flex items-center gap-3 bg-white">
                            ...
                            <Input 
                                ...
                                className="bg-zinc-50 border-none rounded-full h-11 focus-visible:ring-1 focus-visible:ring-yellow-400"
                                value={newMessage}
                    ...
                    <div className="flex-1 flex flex-col items-center justify-center text-zinc-300 bg-zinc-50/30">
                        ...
                        <p className="text-lg font-bold text-zinc-800">Your Messages</p>
                        <p className="text-sm text-zinc-400 mt-2">Select a friend or chat to start</p>
                    </div>
                )}
            </div>
        </div>
