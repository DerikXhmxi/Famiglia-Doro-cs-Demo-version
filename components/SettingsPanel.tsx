...
import { 
  ...
  ChevronRight, ChevronLeft, Camera, Search, Mic, MapPin, UploadCloud, 
  PlusCircle, Ban, Check, LogOut, ShieldAlert, DollarSign, TrendingUp,
  ArrowUpRight, ArrowDownLeft, Trash2, Loader2, Info, Smartphone, Globe,
  ShoppingBag, Ticket, Calendar, Clock, Receipt
} from 'lucide-react'
...
function NotificationSettings({ session }: { session: any }) {
  ...
  const toggle = async (key: keyof typeof settings) => {
    ...
    if ((key === 'messages' || key === 'activity') && newSettings[key]) {
...
function PermissionsSettings({ session }: { session: any }) {
  ...
  const requestAccess = async (key: string) => {
      try {
          ...
          if (key === 'contacts') {
              alert("Contact access is managed by your device OS settings.")
              return true 
          }
      } catch (err) {
          console.error(err)
          alert("Permission Denied. Please allow access in your browser address bar.")
          return false
      }
      return false
  }
  ...
  const toggle = async (key: string) => {
      const isCurrentlyOn = perms[key as keyof typeof perms]
      let newState = !isCurrentlyOn

      if (newState === true) {
          const granted = await requestAccess(key)
          if (!granted) newState = false 
      }

      const newPerms = { ...perms, [key]: newState }
      setPerms(newPerms)
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-300">
      ...
      <CustomModal isOpen={isDialogOpen} onClose={() => setIsDialogOpen(false)} title="Add Payment Method">
            <div className="mb-6 w-full aspect-[1.58/1] rounded-2xl p-6 relative overflow-hidden shadow-xl flex flex-col justify-between bg-zinc-900 text-white transition-all">
                ...
                <div className="absolute top-[-50%] right-[-50%] w-full h-full bg-gradient-to-b from-white/10 to-transparent rounded-full blur-3xl"></div>
                ...
                <div className="relative z-10 space-y-4">
                    ...
                    <div className="flex justify-between items-end">
                        <div>
                            <div className="text-[8px] uppercase opacity-50 tracking-wider">Card Holder</div>
                            <div className="font-bold text-sm tracking-wide uppercase truncate max-w-[150px]">{cardName || 'YOUR NAME'}</div>
                        </div>
                        <div>
                            <div className="text-[8px] uppercase opacity-50 tracking-wider">Expires</div>
                            <div className="font-bold text-sm tracking-wide">{cardExpiry || 'MM/YY'}</div>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div className="space-y-4">
                <div className="space-y-2">
                    <label className="text-xs font-bold text-zinc-500 ml-1">Card Number</label>
                ...
                <div className="space-y-2">
                    <label className="text-xs font-bold text-zinc-500 ml-1">Cardholder Name</label>
                ...
                <div className="flex gap-3">
                    <div className="flex-1 space-y-2">
                        <label className="text-xs font-bold text-zinc-500 ml-1">Expiry</label>
                    ...
                    <div className="w-1/3 space-y-2">
                        <label className="text-xs font-bold text-zinc-500 ml-1">CVC</label>
                        <Input value={cardCVC} onChange={e => setCardCVC(e.target.value.replace(/\D/g, ''))} placeholder="123" className="bg-white h-11 text-center border-zinc-200" maxLength={3} type="password" />
                    </div>
                </div>
      ...
      {cards.map((card) => (
          <div key={card.id} className="group w-full bg-white border border-zinc-200 rounded-2xl p-4 flex items-center justify-between shadow-sm hover:shadow-md transition-all">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <p className="font-bold text-sm text-zinc-900">{card.card_type} ending in {card.last_4}</p>
                    <p className="text-xs text-zinc-400">Expires 12/28</p>
                </div>
            </div>
            <Button size="icon" variant="ghost" className="opacity-0 group-hover:opacity-100 transition-opacity text-red-500 hover:bg-red-50" onClick={() => deleteCard(card.id)}>
      ...
      {cards.length > 0 && (
          <div className="bg-blue-50 p-4 rounded-xl flex items-center gap-3 text-blue-700 text-xs border border-blue-100">
              <ShieldAlert className="w-5 h-5 flex-shrink-0"/> 
              <span>Your default payment method is used for subscriptions.</span>
          </div>
      )}
    </div>
  )
        ...
        <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4">
            <div className="bg-zinc-900 rounded-2xl p-6 text-white flex justify-between items-center shadow-lg">
                <div>
                    ...
                    <p className="text-xs text-zinc-400">Track all your purchases and tickets.</p>
                </div>
            ...
            <div className="space-y-4">
                ...
                {!loading && orders.length === 0 && (
                    <div className="text-center py-10 bg-zinc-50 rounded-2xl border border-zinc-100">
                        ...
                        <p className="text-sm font-bold text-zinc-500">No purchases yet</p>
                    </div>
                )}
                ...
                {orders.map((order) => (
                    <div key={order.id} className="bg-white p-4 rounded-2xl border border-zinc-100 shadow-sm flex items-center justify-between group hover:border-yellow-200 transition-colors">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <div className="flex items-center gap-2 text-xs text-zinc-500 mt-0.5">
                                    <span className="capitalize bg-zinc-100 px-1.5 py-0.5 rounded">{order.item_type}</span>
                                    <span>•</span>
                        ...
                        <div className="text-right">
                            <p className="font-black text-zinc-900">${order.amount}</p>
                            <span className="text-[10px] font-bold text-green-600 bg-green-50 px-2 py-0.5 rounded-full border border-green-100 uppercase tracking-wide">Paid</span>
                        </div>
                    </div>
                ))}
            </div>
    ...
    useEffect(() => {
        const fetchBlocked = async () => {
            const { data } = await supabase
                .from('blocked_users')
                .select(`id, blocked_id, profiles:blocked_id ( username, avatar_url )`)
...
export default function SettingsPanel({ onClose, onLogout }: { onClose: () => void, onLogout: () => void }) {
  ...
  const menuItems = [
    ...
    { id: 'history', label: 'Purchase History', icon: Receipt, component: () => <PurchaseHistory session={session} /> }, // NEW TAB
